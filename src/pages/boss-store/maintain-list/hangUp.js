...
class HangUp extends Component {
  ...
  inStore = () => {
    ...
    router.push("/boss-store/purchase")
  }
  goDetail = (orderId, orderType) => {
    const { dispatch } = this.props
    dispatch({
      type: "maintianList/goDetail",
      payload: { orderId, orderType },
    })
    router.push("/boss-store/maintain-list")
  }
  getData = (boolean) => {
    let o = {}
    const { dispatch } = this.props
  ...
  unHangOrder = (id) => {
    services
      .UPDATE({ keys: { name: "maintain/unHangup", id }, data: { operate: 2 } })
      .then((res) => {
        if (res.success) {
          message.success("订单已取消挂起")
          this.getData()
        } else {
          message.info("网络出现错误")
        }
                ...
                >
                  ...
                  >
                    ...
                    <ul>
                      ...
                      <li onClick={this.unHangOrder.bind(this, record.orderId)}>
                        <p>取消挂起</p>
                      </li>
                    </ul>
                  </div>
...
export default connect(mapStateToProps)(HangUp)
