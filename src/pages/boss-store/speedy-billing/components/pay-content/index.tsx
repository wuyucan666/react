...
class payModal extends Component<pageProps> {
  ...
  async componentWillMount () {
    ...
    if(res.success) {
      ...
      dispatch({ type: 'speedyBilling/setOrder', payload: {
        payList,
        goodsList,
        amount: data.amount,
  ...
  async cardPay(payment: any) {
    ...
    payList.map((v: any) => {
      if(v.clientCardId === payment.clientCardId) {
        if(v.balance >= amount) {
          ...
          dispatch({type: 'speedyBilling/setOrder', payload:{ pendingPay: v.checked ? 0 : amount }})
        }else {
    ...
    await dispatch({type: 'speedyBilling/setOrder', payload:{ payList }})
  ...
  goMix = (payment: any, isCard: boolean) => {
    ...
    this.setState({payList})
    dispatch({type: 'speedyBilling/setOrder', payload:{ payList, pendingPay: fromOrder.amount }})
  }
