 ...
 */
  ...
  // 筛选选择会员
  onChangeSms=(value) => {
    let index = this.state.screenArr.findIndex(v => v.id === value)
    if(index < 0) {
      let obj = this.state.membeArr.find(v => v.id === value)
      let screenArr = this.state.screenArr
      screenArr.push({...obj})
      this.setSearch({screenArr})
    } else {
      message.warn('该会员已经选择，不可重复选择！')
    }
    this.setState({membeArr: this.state.peopleArr.map(_ => ({..._}))})
  }
  // 筛选会员
  onSearchSms=(value) => {
    let arr = this.state.peopleArr.map(v => ({...v}))
    let membeArr = []
    for(let i in arr) {
      if((arr[i].clientName && arr[i].clientName.indexOf(value) >= 0) || (arr[i].phone && (arr[i].phone+'').indexOf(value) >= 0)) {
        membeArr.push({...arr[i]})
      } else {
        for(let j in arr[i].car) {
          let v = arr[i].car[j]
  ...
  setSearch=(arr) =>{
    ...
    this.setState({membeArr})
  }
  // 删除已筛选的会员
  chooesDel=(index)=> {
    let screenArr = this.state.screenArr.map(v => ({...v}))
    screenArr.splice(index, 1)
    this.setState({screenArr})
  }
  render() {
    const {content,time,maxLength,sendType,sms,visible,allCheck,sellMin,sellMax,timeMax,timeMin,smsAmount,title, peopleNum,checkPeople,dressPerson,membeArr, screenArr} = this.state    //,screenArr
    const {styles, onCancel} = this.props
    return (
      <div className={styles.newMass}>
        <div>
           <div>
            ...
            <div  className={styles.formItem}>
              <Button type='primary' style={{height:'40px'}} disabled={checkPeople>smsAmount} onClick={this.submit.bind(this)}>确认</Button>
              <Button size="large" onClick={onCancel} style={{ verticalAlign: 'top', marginLeft: '20px' }}>取消</Button>
          ...
          <Modal
            ...
            maskClosable={false}
          >
          {!allCheck&&<div style={{marginBottom: '20px'}}>
            <Select
              showSearch
              style={{width:'100%'}}
              value={this.state.value}
              placeholder='会员名称/手机号/车牌号'
              defaultActiveFirstOption={false}
              showArrow={false}
              size='large'
              filterOption={false}
              onSearch={this.onSearchSms}
              onChange={this.onChangeSms}
              notFoundContent={null}
            >
              {membeArr.map(item => <Option key={item.id} value={item.id}>{item.clientName}</Option>)}
            </Select>
          </div>}
          ...
          {!allCheck&&<div style={{border:'1px solid #78B6E8',padding:'16px 20px 20px',marginTop:'10px'}}>
            ...
            <div><Button onClick={this.screen.bind(this)} style={{marginRight:'5px', width: '88px', height: '30px'}} type='primary' size='small'>筛选</Button>筛选结果<span style={{color:'#4aacf7',marginLeft:'5px'}}>{dressPerson}</span>&nbsp;人</div>
          </div>}
          {!allCheck&&<div className={styles.chooes_item}>已选群发会员<div>共<span> {screenArr.length} </span>人</div></div>}
          {!allCheck&&<div className={styles.chooes_box}>
            <div>
              {screenArr && screenArr.map((item, index) => {
                return (
                  <div className={styles.chooes_c} key={index}>
                    <div className={styles.chooes_c_name}>{item.clientName}</div>
                    <div className={styles.chooes_c_p}>{
                      item.car && item.car.map((v, ind) => <span key={ind}>{v}{ind === (item.car.length - 1) ? '' : ' / '}</span>)
                    }</div>
                    <div className={styles.chooes_c_d} onClick={this.chooesDel.bind(this, index)}><Icon type="close" /></div>
                  </div>
                )
              })}
            </div>
