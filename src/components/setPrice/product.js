...
class SettingProductPrice extends Component {
  ...
  componentDidMount() {
    this.props.dispatch({ type: "storeSort/getRole", payload: {} })
  }
  ...
  sellChange = (value) => {
    ...
    if (consType * 1 === 2) { //原价
      this.setState({ consMoney: (value * 1 * consPercent / 100).toFixed(1) })
    } else {
      this.setState({ consMoney: 0 })
    }

    if (saleType * 1 === 2) {
      this.setState({ saleMoney: (value * 1 * salePercent / 100).toFixed(1) })
    } else {
      this.setState({ saleMoney: 0 })
    }
  }
  ...
  onOk = () => {
    ...
    form.validateFields((err, values) => {
      if (err) {
        ...
        values = {
          ...values,
          productId: curItem.id || curItem.pId,
      ...
      service.INSERT({
        keys: { name: 'store/product/update' },
        data: values,
      }).then(res => {
        if (res.success) {
          onClose(true, { ...curItem, price: values.sellingPriceTem, info: 0, itemTotal: values.sellingPriceTem * curItem.num * curItem.discount/10})
  ...
  render() {
    ...
    const {category,curItem}=this.props
    let idArr=[]
      //处理分类tree，初始化
    if(category){category.map((item)=>{
        if(item.children){
          ...
          <Form >
            ...
            >
              {getFieldDecorator('productName', {
                ...
                <Input size='large' placeholder="产品名称" disabled></Input>
              )}
            </FormItem>
            ...
            >
              {getFieldDecorator('commodityCode', {
                ...
                initialValue: `${this.props.curItem.commodityCode}`,
              })(
                <Input size='large' placeholder="产品编码" disabled={this.props.curItem.formTem*1 === 1}></Input>
              )}
            </FormItem>
            <FormItem
              label="售价"
            ...
            <FormItem
              ...
              extra={
                ...
                ) : this.state.saleType*1 === 4 && (this.state.saleMoney*1 > getFieldValue('sellingPriceTem') || (this.state.saleMoney*1 && !getFieldValue('sellingPriceTem'))) ? (
            ...
            <FormItem
              ...
              extra={
                ...
                ) : this.state.consType*1 === 4 && (this.state.consMoney*1 > getFieldValue('sellingPriceTem') || (this.state.consMoney*1 && !getFieldValue('sellingPriceTem'))) ? (
                <div style={{color: '#FF6F28', fontSize: '12px'}}>
                  提成金额大于售价，请留意
                </div>
...
function mapStateToProps(state) {
  const {  category } = state.storeSort
  return { category }
}
const WrappedTimeRelatedForm=Form.create()(SettingProductPrice)
