  ...
  class extends Component {
    ...
    handleSubmit() {
      ...
      validateFields((err, values) => {
        ...
        if (type === "add") {
          services
            .INSERT({ data: values, keys: { name: "brand/project/addition" } })
            .then(res => {
              if (res.success) {
                message.success("添加成功!")
                dispatch({
                  type: "table/getData",
                  payload: { new: "brand/project/addition" },
                })
                hideModal()
              } else {
          ...
          services
            .UPDATE({ data: values, keys: { name: "brand/project/addition", id: editItem.projectId } })
            .then(res => {
              if (res.success) {
