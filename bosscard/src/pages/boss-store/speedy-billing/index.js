...
class Index extends Component {
  ...
  componentDidMount() {
    ...
    if (isAdding) {
      //用户其他操作,清除redux数据
      dispatch({ type: "speedyBilling/resetState" })
  ...
  createData = (type, callBack) => {
    ...
    if (orderInfo.orderId) {
      ...
      this.setState({loading: true}, () => {
        service
          .UPDATE({
            keys: { name: "quick/order", id: orderInfo.orderId },
            data: obj,
          })
          .then((res) => {
            loading()
      ...
      this.setState({loading: true}, () => {
        service
          .INSERT({ keys: { name: "quick/order" }, data: obj })
          .then((res) => {
            loading()
            this.setState({loading: false})
            if (res.success) {
  ...
  onOk = () => {
    // 修改订单
    // projectInfo = projectInfo.map(v => {
    //   if(v.packageId) { //卡内
    //     return {...v, projectId: v.id, typeId: v.projectId}
    //   }else {
    //     return v
    //   }
    // })
  ...
  onSearchChange = (value) => {
    ...
    const { prevMileage, clientId, carId } = value
    ...
    dispatch({
      type: "speedyBilling/addGuide",
      payload: {
        clientId,
      },
    })
    dispatch({
      type: "speedyBilling/addPackage",
      payload: {
        clientId,
        carId,
      },
  ...
  goBack = () => {
    const { dispatch } = this.props
    dispatch({
      type: "speedyBilling/modification",
    })
    this.layout.current.setCurrent(-1)
  }
  // 修改订单
  ...
  handleOk = () => {
    ...
    // if (debt) {
    //   router.push({ pathname: "/boss-store/maintain-list/buying" })
    // } else {
    //   router.push({ pathname: "/boss-store/maintain-list/doing" })
    // }
  }
  ...
  render() {
    ...
    const { printData, showPrint, allCardGoods, isZero, showTip, printType, showCarCheck, showChecks, showPrintCarInspection, printCarData, showPrintCityOrder, printCityOrderData} = this.state
    let isNeedIn = false
    let isNeedSet = false
    ...
    if (!parkInfo.counselor) tips = "请填写接车信息!"
          ...
          operation={[
            {
              ...
              hidden: projectInfo.length === 0,
              handel: () => { this.setState({ showBulkChoose: true, curType: 1 }); collectData('order')},
            },
          ]}
          >
            <AddProject
        ...
        main: (
          <Item title="添加产品"
            operation={[
              ...
              {
                name: <Button type='primary' size='large' onClick={() => this.setState({showStorageIn: true})} disabled={productInfo.filter(v => v.num > v.stock).length === 0}>批量入库</Button>,
                hidden: productInfo.length === 0,
