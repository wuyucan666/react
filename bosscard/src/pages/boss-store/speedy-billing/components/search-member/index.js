...
class SearchMember extends Component {
  ...
  getRecord = (clientId) => {
    services.LIST({keys: {name: 'maintain/recently'},data: {q: {page:1,limit: 2,where: {clientId}}}}).then(res => {
      if(res.success){
        console.log(res.list, 777777777)
      ...
      <div className={style.searchMember}>
        ...
        {!!carId && (
          <div className={style.client} onClick={this.clickAccount}>
            <div className='head'>
              <div style={{display:clientInfo.icon?'':'none'}}>
                <img src={Array.isArray(clientInfo.icon) ?(clientInfo.icon?clientInfo.icon[0].requestAddress:'') :(clientInfo.icon?clientInfo.icon.requestAddress:'')} alt=' '/>
              </div>
